        ...
        private void LoadmyBookings()
        {
            ...
            try
            {
                using (SqlConnection conn = DatabaseConnection.GetConnection())
                {
                    ...
                    string query = @"SELECT B.BookingID, C.Camera_Name, A.Accessory_Name, L.Lenses_name, B.StartDate, B.EndDate, B.Booking_Status 
                                     FROM Bookings B 
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            ...
                            adapter.Fill(dt);
                            myBookingGridView.DataSource = dt;
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show("Error loading your bookings: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
