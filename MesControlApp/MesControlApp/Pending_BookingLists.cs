        ...
        private void LoadPendingBookings()
        {
            try
            {
                using (SqlConnection conn = DatabaseConnection.GetConnection())
                {
                    ...
                    string query = "SELECT BookingID, BookingDate, BookingTime, BookingStatus FROM Bookings WHERE BookingStatus = 'Pending'";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            ...
                            adapter.Fill(dt);
                            dataGridViewPendingBookings.DataSource = dt; // Ensure DataGridView is named dataGridViewPendingBookings
                        }
                    }
                }
            }
